...
static void* yabai_proxy_begin_proc(void* context) {
  ...
  if (!proxy->is_proxy) {
    ...
    border_update_internal(proxy, &info->settings, NULL);
  }
...
static void* yabai_proxy_end_proc(void* context) {
  ...
  border_update_internal(border, &info->settings, NULL);
...
static inline void yabai_proxy_begin(struct table* windows, uint32_t wid, uint32_t real_wid) {
  ...
  if (border) {
    ...
    if (!border->proxy) {
      ...
      border_create_window(border->proxy, CGRectNull, true, border_get_settings(border)->hidpi);
